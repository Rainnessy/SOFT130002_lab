...
6. As you can see, we provided two ways to accessing database. Please tell me what two pieces of code above did and how they worked (In other words, explain the code above).  
   ...
   lab10-pdo是pdo访问数据库，抛出异常类和错误信息。查询Artist顺序是由LastName决定的，输出每行的ArtistID-GenreID.<br/>
   lab10-mysqli是mysqli访问数据库，同样也抛出异常信息，查询Genres按GenreName，这些信息保存在一个下拉菜单中
...
5. Explain three functions in `lab10-exercise8.php` and write it in the document.  Please also attach the screenshot in the document.  
    outputArtists()向数据库查询0，30的艺术家，并设置$_GET['id'],如果id被设置且等于ArtistID，则超链接的类名加上active.正常情况下类名仅有item,超链接的
                   文字是艺术家的lastname
    outputPaintings()向数据库查询ArtistID对应的艺术家的油画作品，取得每一幅油画作品后调用outputSinglePainting($row)输出
    outputSinglePainting($row)就是油画的内容，直接通过键值对输出
![8](screenshots/8.PNG)
...
![9](screenshots/9.PNG)
    pdo的exec方法，即时执行，prepare+execute先预处理再执行
    mysqli_stmt_fetch()方法预处理
    优点：安全性更好，多次提交性能更好
...
![10](screenshots/10.PNG)
    outputGenres向数据库请求GenreId, GenreName, Description排序按GenreId
    outputSingleGenre用刚才的信息生成每张图片基本内容
    constructGenreLink构建 GenreName所指代的内容
